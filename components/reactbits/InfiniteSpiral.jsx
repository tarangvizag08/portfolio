...
// Cards are laid out at RASTER x their size and scaled down, so the GPU layer is
// rasterized at high resolution instead of being upscaled (blurry) when focused.
const RASTER = 2;
  ...
  useEffect(() => {
    ...
    const render = time => {
      ...
      cardRefs.current.forEach((card, index) => {
        ...
        card.style.transform = `translate(-50%, -50%) translate3d(${x}px, ${offset * verticalSpacing * fit}px, 0) rotateZ(${cardTilt}deg) scale(${visualScale / RASTER})`;
        card.style.opacity = opacity.toFixed(3);
        card.style.filter = blur > 0.01 ? `blur(${(blur * RASTER).toFixed(2)}px)` : 'none';
        ...
        {normalizedItems.map((item, index) => {
          ...
          return (
            <Card
              ...
              style={{ width: cardWidth * RASTER, height: cardHeight * RASTER, borderRadius: cardRadius * RASTER }}
              href={item.href}
              target={item.target}
            ...
            >
              <img
                ...
                draggable={false}
                style={{
                  width: cardWidth * RASTER,
                  height: cardHeight * RASTER,
                  maxWidth: 'none',
                  maxHeight: 'none',
                  objectFit: imageFit,
